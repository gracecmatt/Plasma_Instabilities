...
%        [Fp,F]=calFp(F);
       ...
       Zp=calZ(z,Fp,N);
            % with correction:
       Zp=Zp+3*sqrt(3)*vt^2/(4*pi)/(vc^3+vt^3).*(1./(z-vc)-1./(z+vc));
...
function Z=calZ(z,F,N,del)
   ...
   Z=hilb(z,F,N,del);
   ind1=find(isnan(Z));
     % changed from 1e-10 to 1e-14 (gm 6/29/23)
   z1=z(ind1)+1e-14;             % avoid NaN of higher order singular point
...
%   it seems we should set del=0 for correct analytic continuation
    ...
    if nargin<5                                       % optimal choice of L
        L = 1;%sqrt(N/sqrt(2)); 
    end             
    if nargin<4, del = 1; end
    ...
    for id=idx1
        ...
        FF = eval(F);                            % sample the function at v
        FF(isnan(FF))=0;
        Fz = eval(['@(v)',F]);      % define F(z) for analytic continuation
        ...
        a = (fft(fftshift(FF)))/M2;             % coefficients of transform - takes care of signs
        % % removed fftshift (gm 7/14/23); a = fft(FF)/M2;
        a0 = a(1); a = flipud(a(2:N+1));             % reorder coefficients %.*((-1).^(1:N))'
