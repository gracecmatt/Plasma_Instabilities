...
function w = Vlasov_1D_linearized_Steve_v3_Kappa(k, theta, mu, beta, kappa)
...
M=1024/2*2*2;   % 2M is a number of grid points in v-direction 
dv=Vmax/M;
v=(-M:M-1)*dv;
...
% Kappa with bump
C1=(pi*theta(1)^2*(kappa-0.5))^(-1/2)*exp(gammaln(kappa+1)-gammaln(kappa+0.5));
C2=(pi*theta(2)^2*(kappa-0.5))^(-1/2)*exp(gammaln(kappa+1)-gammaln(kappa+0.5));
f0=beta*C1*(1+(v-mu(1)).^2/(kappa-0.5)/theta(1)^2).^(-kappa-1) + ...
(1-beta)*C2*(1+(v-mu(2)).^2/(kappa-0.5)/theta(2)^2).^(-kappa-1);
f0_v=beta*C1*2*(v-mu(1))*(-kappa-1)/(kappa-0.5)/theta(1)^2.*(1+(v-mu(1)).^2/(kappa-0.5)/theta(1)^2).^(-kappa-2) + ...
(1-beta)*C2*2*(v-mu(2))*(-kappa-1)/(kappa-0.5)/theta(2)^2.*(1+(v-mu(2)).^2/(kappa-0.5)/theta(2)^2).^(-kappa-2);
...
%            end
            ...
            id_max_start_w=id_max_start_+d_id-1;  
            
            MAX_count=0;
            FACTOR=2*(gamma>0)+1*(gamma<0);
            id_max_finish_w=id_max_start_w; max_finish_w=max_start_w;
            while id_max_finish_w < n-ceil(FACTOR*3*n_half_period/2)
