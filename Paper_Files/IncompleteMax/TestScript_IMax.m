...
parfor j = 1:N^2
    ...
    xi_guess = init_guess/(randparams(2)*randparams(1)); % shifted & scaled
 
    init = init_guess + randparams(3)*randparams(1);
    ...
    errorRand(j).init = abs(real(dielectric)-real(init))/abs(real(dielectric))+1i*abs(imag(dielectric)-imag(init))/abs(imag(dielectric));
end
delete(gcp('nocreate'));
