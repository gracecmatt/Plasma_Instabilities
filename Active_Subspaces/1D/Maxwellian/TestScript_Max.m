...
for i = 1:Nparams
    ...
    for j=1:N
        ...
        init_guess = BohmGross(k,sigma,0) + mu*k;
        ...
        omega.xie_rescaled(i,j) = Maxwellian_Disp_Using_Xie(k,sigma,0,xi_guess_rescaled)*k + mu*k;
        omega.dielectric(i,j) = Max_dielectric([k,sigma,0],(init_guess-mu*k)/k)*k + mu*k;
    end
...
txtleg1 = {'Bohm-Gross','Xie','Rescaled Xie','Dielectric Func.'};
txtleg2 = {'Bohm-Gross','Xie','Rescaled Xie'};
...
for i=1:Nparams
    ...
    leg = legend(txtleg1,'Orientation','Horizontal');
    leg.Layout.Tile = 'north';
    sgtitle('Maxwellian $\gamma(\vec{p})$ around '+txtbase,'Interpreter','latex','FontSize',16);
...
for i=1:Nparams
    ...
    semilogy(paramsarr(i,:), errorIm.xie_rescaled(i,:),'.-');
        ...
        ylim([0,max(1,max(errorIm.spectral(i,:)))]); grid on;
end
    leg = legend(txtleg2,'Orientation','Horizontal');
    leg.Layout.Tile = 'north';
    sgtitle('Maxwellian Error in $\gamma$ around '+txtbase,'Interpreter','latex','FontSize',16);
...
for i=1:Nparams
    ...
    semilogy(paramsarr(i,:), errorRe.xie_rescaled(i,:),'.-');
        ...
        xlabel('$'+txtparams(i)+'$','Interpreter','latex','FontSize',14);
        ylabel('Error','Interpreter','latex','FontSize',14);
        ylim([0,max(1,max(errorRe.spectral(i,:)))]); grid on;
end
    leg = legend(txtleg2,'Orientation','Horizontal');
    leg.Layout.Tile = 'north';
    sgtitle('Maxwellian Error in $\Omega$ around '+txtbase,'Interpreter','latex','FontSize',16);

pause(0.5) % pause to let figures show up
%% Run simulations N^2 times with randomly selected parameters
rng('shuffle');
tic;
if N==50; parpool(4); elseif N==100; parpool(10); else; parpool(12); end
parfor j = 1:N^2
    ...
    spectral = init_guess;
    xie = Maxwellian_Disp_Using_Xie(randparams(1),randparams(2),0,xi_guess)*randparams(1)...
        + randparams(3)*randparams(1);
    dielectric = Max_dielectric([randparams(1),randparams(2),0],xi_guess)*randparams(1)...
        + randparams(3)*randparams(1);
...
sgtitle(txt,'Interpreter','latex','FontSize',16)
    txt = ['Maximum Relative Error: ',num2str(errorMax)];
