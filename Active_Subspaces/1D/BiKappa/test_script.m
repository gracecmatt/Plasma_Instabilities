...
baseparams = [0.5; 1; 0.6; 0; 2.5; 0.9];
...
for i = 1:Nparams
    ...
    for j=1:N
        ...
        init_guess = Vlasov_1D_linearized_Steve_v4_Kappa(k,sigma1,sigma2,0,mu2-mu1,beta,kappa); %tilde{Omega}+igamma

        xi_guess = (init_guess+mu1*k)/k;
        xi_guess_rescaled = init_guess/(sigma1*k);

        omega.spectral(i,j) = init_guess+mu1*k; 
        omega.xie(i,j) = Kappa_Bump_Disp_Using_Xie(k,sigma1,sigma2,mu1,mu2,beta,kappa,xi_guess)*k;
        omega.xie_rescaled(i,j) = Kappa_Bump_Disp_Using_Xie(k*sigma1,1,sigma2/sigma1,0,(mu2-mu1)/sigma1,beta,kappa,xi_guess_rescaled)*sigma1*k + mu1*k;
        omega.dielectric(i,j) = BiKappa_dielectric(k,sigma1,sigma2,0,mu2-mu1,beta,kappa,init_guess)+mu1*k;
    end
...
for i=1:Nparams
    ...
    plot(paramsarr(i,:), imag(omega.dielectric(i,:)),'k');
        title('BiKappa $\gamma('+txtparams(i)+')$','Interpreter','latex','FontSize',16);
...
for i=1:Nparams
    ...
    plot(paramsarr(i,:), real(omega.dielectric(i,:)),'k');
        title('BiKappa $\Omega('+txtparams(i)+')$','Interpreter','latex','FontSize',16);
        xlabel('$'+txtparams(i)+'$','Interpreter','latex','FontSize',14);
...
parfor j = 1:N^2
    ...
    init_guess = Vlasov_1D_linearized_Steve_v4_Kappa(randparams(1),randparams(2),randparams(3),0,randparams(5)-randparams(4),randparams(6),kappa); %tilde{Omega}+igamma
    xi_guess = init_guess/(randparams(2)*randparams(1));

    spectral = init_guess + randparams(4)*randparams(1);
    xie = Kappa_Bump_Disp_Using_Xie(randparams(1)*randparams(2),1,randparams(3)/randparams(2),0,(randparams(5)-randparams(4))/randparams(2),randparams(6),kappa,xi_guess)*randparams(2)*randparams(1) + randparams(4)*randparams(1); %omega=xi*sigma*k+mu*k
...
txt = ['BiKappa, ',num2str(var*100),'\% variation on $(k,\sigma_1,\sigma_2,\mu_1,\mu_2,\beta)$=(',...
