...
%close all                  
Vmax =130;  % choose Vmax so that f0(Vmax) < 1e-16           
...
% f0_v = f_gauss((v-mu)/sigma).*(-2*(v-mu)/sigma^2)/sigma; 

% Lorentzian
% f0 = 1/pi*sigma./((v-mu).^2+sigma^2);
% f0_v = 1/pi*sigma*(-2*(v-mu))./((v-mu).^2+sigma^2).^2;

% BiLorentzian
f0 = beta/pi*sigma1./((v-mu1).^2+sigma1^2) + (1-beta)/pi*sigma2./((v-mu2).^2+sigma2^2);
f0_v = beta/pi*sigma1*(-2*(v-mu1))./((v-mu1).^2+sigma1^2).^2 + (1-beta)/pi*sigma2*(-2*(v-mu2))./((v-mu2).^2+sigma2^2).^2;
