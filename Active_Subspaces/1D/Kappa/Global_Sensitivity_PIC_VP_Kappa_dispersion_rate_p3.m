...
setvals = [0.5; 1; 0];
beta = 1; % no bump
mu2 = 0; theta2 = 1; % not used, but needed for dielectric_kappa()
...
parfor jj = 1:N
    ...
    init_guess = Vlasov_1D_linearized_Steve_v4_Kappa(params(1), params(2), params(3), kappa);
    % growth(jj) = Kappa_Disp_Using_Xie(params(1), params(2), params(3), kappa, init_guess);
    growth(jj) = dielectric_kappa(params(1), params(2),theta2,params(3),mu2,beta,kappa,init_guess); 
...
parfor jj = 1:N
    ...
    for kk = 1:Nparams
        ...
        init_guess_plus = Vlasov_1D_linearized_Steve_v4_Kappa(paramsplus(1), paramsplus(2), paramsplus(3), kappa);
        % growth_plus(jj, kk) = Kappa_Disp_Using_Xie(paramsplus(1), paramsplus(2), paramsplus(3), kappa, init_guess_plus);
        growth_plus(jj,kk) = dielectric_kappa(paramsplus(1), paramsplus(2),1,paramsplus(3),0,beta,kappa,init_guess_plus); 
...
%Save the trial data
save(['Data/kappa',int2str(kappa),'/Dispersion_Rate_Kappa_P' int2str(Nparams) '_N' int2str(N) '_' num2str(var) 'data_par.mat'])
