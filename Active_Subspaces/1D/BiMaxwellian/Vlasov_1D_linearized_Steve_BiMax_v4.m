...
% Assume f(x,v)=df(v)*exp(i*k*x) - small disturbance with just one k-vector
% Solve df_t + v*df_x - E*f0_v= Dvvvv*(f-<f>)_vvvv on [0,L]x[-Vmax,Vmax] with periodic boundary conditions in x and v,
% -k^2*phi=phi_xx=-rho=-Integral(df,-inf,inf)dv ~ exp(i*k*x) and 
% E=d(phi)/dx=i/k*rho
% So we solve
% df_t + i*k*v*df - i/k*rho*f0_v= Dvvvv*(f-<f>)_vvvv on [0,L]x[-Vmax,Vmax] with periodic boundary conditions in x and v

% Using Split Step 2nd Order 
% and Exact solution for both of terms

% for Maxwell background exact solutions for different k are (gamma=Im[w]=Landau damping): 
% k=0     w_true=1
...
% Output of this function is w = Re[w]+1i*Im[w]

function w = Vlasov_1D_linearized_Steve_BiMax(p)
%close all                  
...
Vmax = 8 + max(abs(mu1), abs(mu2)); % choose Vmax so that f0(Vmax) < 1e-16           
L=2*pi/k; % size of the system in x-direction
N=1;      % 2N is a number of grid points in x-direction
M=2^8;    % 2M is a number of grid points in v-direction 
dv=Vmax/M;
v=(-M:M-1)*dv;

dt=0.001;                        % Time step. Error in gamma is ~ dt^2
T_recurrence=min(L/dv,1000);
tfinal=T_recurrence*0.4;        % Stops the simulation before T_recurrence  
T_period=2*pi/sqrt(1+4*k^2);    % We need approximate value of one period, T _period=2pi/Re[w] for Maxwellian f0(v)=exp(-v^2/2)/sqrt(2pi) 
n_half_period=floor(T_period/2/dt); % Approximate half-period in terms of time steps dt 
t_first_measure=T_recurrence*0.3; % 20*8;           % Time of "the first measurement". Make sure the transient dynamics in df(v) is gone after t_first_measure
n_first_measure=ceil(t_first_measure/dt);
nsteps = round(tfinal / dt);    % number of time steps
nplot = 100000;                 % plot solution every nplot time steps
...
sigma1=sqrt(sigma2_1);     sigma2=sqrt(sigma2_2);
...
% Initial Condition for df(v)
% Gauss
% df=f_gauss(v); % gives best accuracy compared to the other options
% sigma_IC=1;%Vmax/10;
...
CutV=true; % Additional filtering at tails in v-space. Helps when initial df(v) does not decay to 0 at v->Vmax
...
Dvvvv=abs(gamma_true)*(Vmax*2/M/pi)^4; % This is roughly what we need to kill recurrance for gamma=0.029
Dvvvv=0;
...
%         title(sprintf('df_p(t=%g)  after %4i time steps with %ix%i grid points, T_{recurrence}=%g', t,n,N,2*M, L/dv))
        ...
        %     if id_max_start==0
        %         slope=E_A(n_first_measure)-E_A(n_first_measure-1);
        %         if slope<0
        %             T_move=ceil(n_half_period/2);
        %         else
        ...
        % 
        %     % Compute Re[w] by counting the number of maximums (half-periods for gamma<0 and full periods for gamma>0) between t_start=id_max_start*dt and t_finish=id_max_finish*dt
        %     [max_start_w, d_id]=max(E_A(id_max_start-T_move:id_max_start).*exp(-dt*((id_max_start-T_move:id_max_start)-1/2)*gamma));
        %     id_max_start_w=id_max_start-T_move+d_id-1;  
        % 
        %     MAX_count=0;
        %     FACTOR=2*(gamma>0)+1*(gamma<0);
        %     id_max_finish_w=id_max_start_w; max_finish_w=max_start_w;
        %     while id_max_finish_w < n-ceil(FACTOR*n_half_period/2)
        %         id_start=id_max_finish_w + floor(FACTOR*n_half_period/2);
        %         id_stop=min(id_max_finish_w + floor(FACTOR*3*n_half_period/2),n);
        %         [max_finish_w, d_id]=max(E_A(id_start:id_stop).*exp(-dt*((id_start:id_stop)-1/2)*gamma));
        %         id_max_finish_w=id_start+d_id-1;  
        %         MAX_count=MAX_count+1;
        %     end
        %     % Compute w=Re[w]+1i*Im[w]
        %     w=FACTOR*pi*MAX_count/((id_max_finish_w-id_max_start_w)*dt)+gamma*1i;
        % 
        %     % figure(5);
        ...
        %     % title(sprintf('E(t=%g)  after %4i time steps with %ix%i grid points    w=%1.8g+%0.7g*i', t,n,N,2*M,real(w),imag(w)))
        %     % 
        %     % figure(6); % compensated for gamma decay/growth - has to be close to horizontal line
        %     % semilogy(dt*((0:n-1)+1/2),E_A(1:n).*exp(-dt*((0:n-1)+1/2)*gamma),[id_max_finish-1/2 id_max_start-1/2]*dt,[max_finish max_start].*exp(-[id_max_finish-1/2 id_max_start-1/2]*dt*gamma),'r*',[id_max_finish_w-1/2 id_max_start_w-1/2]*dt,[max_finish_w max_start_w],'m*');
        %     % title(sprintf('E(t=%g)*exp(-gamma*t)  after %4i time steps with %ix%i grid points    w=%1.8g+%0.7g*i', t,n,N,2*M,real(w),imag(w)))
        % end
        % % pause(0.01)
        % Compute w=Re[w]+1i*Im[w]
        if n>n_first_measure
