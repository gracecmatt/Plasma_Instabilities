...
% This script does not yet have a "true" solution.
clear; clc;
...
baseparams = [0.5; 1; 0; -1];
txtparams = ["k","\sigma","\mu","\nu"];

% set variations
var = 0.30; % x 100% variation considered
xl = (1-var)*baseparams;
xu = (1+var)*baseparams;
...
for i = 1:Nparams
    ...
    for j=1:N
        ...
        omega.xie_rescaled(i,j) = IncompleteMax_Disp_Using_Xie(sigma*k,1,0,(nu-mu)/sigma,xi_guess_rescaled)*k*sigma + mu*k;
...
for i=1:Nparams
    ...
    % plot(paramsarr(i,:), imag(omega.dielectric(i,:)),'k');
        title('Incomplete Max $\gamma('+txtparams(i)+')$','Interpreter','latex','FontSize',16);
...
for i=1:Nparams
    ...
    % plot(paramsarr(i,:), real(omega.dielectric(i,:)),'k');
        title('Incomplete Max $\Omega('+txtparams(i)+')$','Interpreter','latex','FontSize',16);
        xlabel('$'+txtparams(i)+'$','Interpreter','latex','FontSize',14);
