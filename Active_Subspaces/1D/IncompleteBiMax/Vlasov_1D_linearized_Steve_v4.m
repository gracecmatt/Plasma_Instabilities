...
function w = Vlasov_1D_linearized_Steve_v4(k, sigma1, sigma2, mu1, mu2, beta, nu, Mstep)
...
f0 = beta*(1+erf(Mstep*(v-nu))).*exp(-(v-mu1).^2/sigma1^2)/(sqrt(pi*sigma1^2)*(1+erf(Mstep*(mu1-nu)/sqrt(1+Mstep^2*sigma1^2))))...
+(1-beta)*(1+erf(Mstep*(v-nu))).*exp(-(v-mu2).^2/sigma2^2)/(sqrt(pi*sigma2^2)*(1+erf(Mstep*(mu2-nu)/sqrt(1+Mstep^2*sigma2^2))));

f0_v = beta*( (2*exp(-(v-mu1).^2/sigma1^2))/(sqrt(pi*sigma1^2)+erf(Mstep*(mu1-nu)/sqrt(1+Mstep^2*sigma1^2))) .* ...
    (Mstep*exp(-Mstep^2*(nu-v).^2)/sqrt(pi) - (v-mu1).*(1+erf(Mstep*(v-nu)))/sigma1^2) + ...
    (2*Mstep*exp(-Mstep^2*(v-nu).^2-((v-mu1).^2/sigma1^2)))/(sqrt(pi)*(sqrt(pi*sigma1^2)*(1+erf(Mstep*(mu1-nu)/sqrt(1+Mstep^2*sigma1^2))))) - ...
    2*(v-mu1).*(1+erf(Mstep*(v-nu))).*exp(-(v-mu1).^2/sigma1^2)/(sigma1^2*(sqrt(pi*sigma1^2)*(1+erf(Mstep*(mu1-nu)/sqrt(1+Mstep^2*sigma1^2))))) )...
    + (1-beta)*( (2*exp(-(v-mu2).^2/sigma2^2))/(sqrt(pi*sigma2^2)+erf(Mstep*(mu2-nu)/sqrt(1+Mstep^2*sigma2^2))) .* ...
    (Mstep*exp(-Mstep^2*(nu-v).^2)/sqrt(pi) - (v-mu2).*(1+erf(Mstep*(v-nu)))/sigma2^2) + ...
    (2*Mstep*exp(-Mstep^2*(v-nu).^2-((v-mu2).^2/sigma2^2)))/(sqrt(pi)*(sqrt(pi*sigma2^2)*(1+erf(Mstep*(mu2-nu)/sqrt(1+Mstep^2*sigma2^2))))) - ...
    2*(v-mu2).*(1+erf(Mstep*(v-nu))).*exp(-(v-mu2).^2/sigma2^2)/(sigma2^2*(sqrt(pi*sigma2^2)*(1+erf(Mstep*(mu2-nu)/sqrt(1+Mstep^2*sigma2^2))))) );
