function [Zp]=zetaph_IBiMax(z,Fn,F,N,sigma1,sigma2,mu1,mu2,beta,nu)
...
%        Zp=calZ(z,Fp,N);
    elseif(Fn==5) % incomplete Maxwellian distribution
       ...
       Fp = ['-4*heaviside(real(v)-',num2str(nu,16),').*( ',num2str(beta,16),'.*(v-',num2str(mu1,16),').*exp(-(v-',num2str(mu1,16),').^2./',num2str(sigma1,16),'^2)./sqrt(pi*',num2str(sigma1,16),'^6)./(1+erf((',num2str(mu1,16),'-',num2str(nu,16),')/',num2str(sigma1,16),'))'... 
            '+ ',num2str(1-beta,16),'.*(v-',num2str(mu2,16),').*exp(-(v-',num2str(mu2,16),').^2./',num2str(sigma2,16),'^2)./sqrt(pi*',num2str(sigma2,16),'^6)./(1+erf((',num2str(mu2,16),'-',num2str(nu,16),')/',num2str(sigma2,16),')) )'];
       % [Fp,F]=calFp(F);
       % Z=calZ(z,F,N);
       Zp=calZ(z,Fp,N);
       Zp=Zp+2/(nu-z)*( beta*exp(-(nu-mu1)^2/sigma1^2)/sqrt(pi*sigma1^2)/(1+erf((mu1-nu)/sigma1)) ...
                  + (1-beta)*exp(-(nu-mu2)^2/sigma2^2)/sqrt(pi*sigma2^2)/(1+erf((mu2-nu)/sigma2)) );
