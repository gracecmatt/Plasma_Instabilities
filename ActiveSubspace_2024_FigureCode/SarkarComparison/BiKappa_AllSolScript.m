...
% imaginary part of roots
figure
plot(paramARR,imag(omegaExact),'.-','linewidth',1); hold on
plot(paramARR,imag(omegaXieShiftScaled),'.-','linewidth',1);
plot(paramARR,imag(omegaAApprox),'.-','linewidth',1);
title("Kappa Bump-on-Tail: All Roots $\gamma(k)$",'FontSize',14)
% lgd = legend(strcat(num2str((1:numSol)')),'Location','eastoutside');
% title(lgd,'Solution #'); 
xlabel("$k$"); ylabel("$\gamma(k)$");
...
title('\textbf{Kappa Bump-on-Tail $\gamma(k)$ Relative Error}','FontSize',16);
    ...
    saveas(gcf, 'Figs/gammaRelErrork_Sarkar4.pdf')
    saveas(gcf, 'Figs/gammaRelErrork_Sarkar4FIG.fig')
