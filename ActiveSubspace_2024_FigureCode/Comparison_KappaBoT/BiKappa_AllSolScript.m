...
% paramARR = linspace(0.1,1-0.001,200);
paramARR = logspace(-1,0,200);

xaxis = "k"; 
filenameEnding = "k.pdf"; 
%%% ---------- end change section ----------
omegaExact = zeros(1,length(paramARR));
...
% imaginary part of roots
loglog(paramARR,imag(omegaExact),'*-k','linewidth',1); hold on
loglog(paramARR,imag(omegaXieShiftScaled),'.','linewidth',1,'MarkerSize',10);
loglog(paramARR,imag(omegaAApprox),'.','linewidth',1,'MarkerSize',10);
title("\textbf{Kappa Bump-on-Tail $\gamma(k)$ Level Curves}",'FontSize',16)
    legend('Exact','Weideman','Approx.','FontSize',12)
    xlabel("$k$",'FontSize',14); ylabel("$\gamma(k)$ (negative log)",'FontSize',14);
    set(gca, 'YGrid', 'on', 'XGrid', 'off', 'YMinorGrid', 'off');
    set(gca,'Yscale','log','XScale','Linear')
    colororder(colorSet([1,1,5]))
...
figure
plot(paramARR,yXie,'.','Color',color,'linewidth',1,'MarkerSize',10); hold on
plot(paramARR,yApp,'-k','linewidth',1); 
% title('Kappa Bump-on-Tail $\gamma(k)$ Relative Error','Interpreter','latex','FontSize',16);
title('\textbf{Kappa Bump-on-Tail $\gamma(k)$ Relative Error}','FontSize',16);
    legend('Weideman Algorithm','Analytic Approximation','Location','NorthEast','FontSize',12);
