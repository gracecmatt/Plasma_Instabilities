...
t_first_measure=T_recurrence*0.2; % Time of "the first measurement". Make sure the transient dynamics in df(v) is gone after t_first_measure
...
% Changed on 8/16/23
% C=(pi*sigma^2*(kappa-1.5))^(-1/2)*exp(gammaln(kappa)-gammaln(kappa-0.5));
% f0=C*(1+(v-mu).^2/((kappa-1.5)*sigma^2)).^(-kappa);
% f0_v=C.*2.*(v-mu).*(-kappa)./((kappa-1.5)*sigma^2).*(1+(v-mu).^2./((kappa-1.5)*sigma^2)).^(-kappa-1);
...
% f0=beta*C1*(1+(v-mu1).^2/((kappa-1.5)*sigma1^2)).^(-kappa) + ...
%    (1-beta)*C2*(1+(v-mu2).^2/((kappa-1.5)*sigma2^2)).^(-kappa);
% f0_v=beta*C1*2*(v-mu1)*(-kappa)/((kappa-1.5)*sigma1^2).*(1+(v-mu1).^2/((kappa-1.5)*sigma1^2)).^(-kappa-1) + ...
%   (1-beta)*C2*2*(v-mu2)*(-kappa)/((kappa-1.5)*sigma2^2).*(1+(v-mu2).^2/((kappa-1.5)*sigma2^2)).^(-kappa-1);

% Kappa with bump at -v0 and +v0
% Changed on 12/5/23
C1=(pi*sigma1^2*(kappa-1.5))^(-1/2)*exp(gammaln(kappa)-gammaln(kappa-0.5));
C2=(pi*sigma2^2*(kappa-1.5))^(-1/2)*exp(gammaln(kappa)-gammaln(kappa-0.5));
f0=beta*C1*(1+(v-mu).^2/((kappa-1.5)*sigma1^2)).^(-kappa) + ...
   (1-beta)*C2*1/2*( (1+(v-(mu+v0)).^2/((kappa-1.5)*sigma2^2)).^(-kappa) + (1+(v-(mu-v0)).^2/((kappa-1.5)*sigma2^2)).^(-kappa) );

f0_v=beta*C1*2*(v-mu)*(-kappa)/((kappa-1.5)*sigma1^2).*(1+(v-mu).^2/((kappa-1.5)*sigma1^2)).^(-kappa-1) + ...
  (1-beta)*C2*1/2*( 2*(v-(mu+v0))*(-kappa)/((kappa-1.5)*sigma2^2).*(1+(v-(mu+v0)).^2/((kappa-1.5)*sigma2^2)).^(-kappa-1) + ...
  2*(v-(mu-v0))*(-kappa)/((kappa-1.5)*sigma2^2).*(1+(v-(mu-v0)).^2/((kappa-1.5)*sigma2^2)).^(-kappa-1));
