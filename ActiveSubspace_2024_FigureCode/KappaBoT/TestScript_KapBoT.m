%% Kappa Bump-on-Tail Test Script for Accuracy of Methods
...
Nparams = 6;
kappa = 2; % kappa = 2 or 4

% parameters = [k, sigma1, sigma2, mu, v0, beta]
baseparams = [0.5; 1; 0.7; 0; 4; 0.85];
txtparams = ["k","\sigma_1","\sigma_2","\mu","v_0","\beta"];

% set variations
var = 0.5; % x 100% variation considered
xl = (1-var)*baseparams;
xu = (1+var)*baseparams;
xu(6) = min(xu(6),0.995); % keep beta < 1
xl(6) = max(xl(6),0.75); % keep beta > 0.5
...
% figure
% sigma1=baseparams(2);sigma2=baseparams(3);mu=baseparams(4);v0=baseparams(5);beta=baseparams(6);
...
% f = @(v) beta*C1*(1+(v-mu).^2/((kappa-1.5)*sigma1^2)).^(-kappa) + ...
%     (1-beta)*C2*( (1+(v-(mu+v0)).^2/((kappa-1.5)*sigma2^2)).^(-kappa) );
% plot(linspace(mu-6,mu+6,1000),f(linspace(mu-6,mu+6,1000)),"Linewidth",2); grid on
%     title('Kappa Bump-on-Tail Distribution','Interpreter','latex','FontSize',14);
%     xlabel('$v$','Interpreter','latex','FontSize',12);
%     ylabel('$f(v)$','Interpreter','latex','FontSize',12);
...
for i = 1:Nparams
    ...
    for j=1:N
        ...
        omega.xie(i,j) = KappaBoT_Disp_Using_Xie(k,sigma1,sigma2,mu,v0,beta,kappa,xi_guess)*k;
        omega.xie_shift(i,j) = KappaBoT_Disp_Using_Xie(k,sigma1,sigma2,0,v0,beta,kappa,xi_guess_shift)*k + mu*k;
        omega.xie_shiftscaled(i,j) = KappaBoT_Disp_Using_Xie(k*sigma1,1,sigma2/sigma1,0,v0/sigma1,beta,kappa,xi_guess_shiftscaled)*sigma1*k + mu*k;
        omega.dielectric(i,j) = KappaBoT_dielectric(k,sigma1,sigma2,0,v0,beta,kappa,init_guess-mu*k)+mu*k;
        % omega.analytic(i,j) = AnalyticApproxKappa(k,sigma1,sigma2,v0,beta,kappa)+mu*k;
     end
...
% errorRe.analytic = abs(real(omega.dielectric)-real(omega.analytic))./abs(real(omega.dielectric));
...
% errorIm.analytic = abs(imag(omega.dielectric)-imag(omega.analytic))./abs(imag(omega.dielectric));

%% Plot Level Curves and Percent Errors
close all
txtbase = string(['$(k,\sigma_1,\sigma_2,\mu,v_0,\beta)$=(',num2str(baseparams(1)),...
...
for i=1:Nparams
    ...
    % plot(paramsarr(i,:), imag(omega.analytic(i,:)),'.-');
        ...
        xlim([paramsarr(i,1),paramsarr(i,end)]);
        colororder(colorSet);
end
    leg = legend(txtleg1,'Orientation','Horizontal','FontSize',8);
    leg.Layout.Tile = 'north';
    txt = 'Kappa Bump-on-Tail $\gamma(p)$ around';
...
for i=1:Nparams
    ...
    % plot(paramsarr(i,:), real(omega.analytic(i,:)),'.-');
    ...
    leg = legend(txtleg1,'Orientation','Horizontal','FontSize',8);
    leg.Layout.Tile = 'north';
    txt = 'Kappa Bump-on-Tail $\Omega(p)$ around';
    sgtitle({txt,txtbase},'Interpreter','latex','FontSize',16);
...
for i=1:Nparams
    ...
    % semilogy(paramsarr(i,:), errorIm.analytic(i,:),'.-');
        ...
        ylim([0,max(1,max(errorIm.xie_shiftscaled(i,:)))]); grid on; 
        xlim([paramsarr(i,1),paramsarr(i,end)]); colororder(colorSet);
end
    leg = legend(txtleg2,'Orientation','Horizontal','FontSize',9);
    leg.Layout.Tile = 'north';
    txt = 'Kappa Bump-on-Tail Error in $\gamma(p)$ around';
    sgtitle({txt,txtbase},'Interpreter','latex','FontSize',16);
...
for i=1:Nparams
    ...
    % semilogy(paramsarr(i,:), errorRe.analytic(i,:),'.-');
        title('Relative Error in $\Omega('+txtparams(i)+')$','Interpreter','latex','FontSize',14);
        xlabel('$'+txtparams(i)+'$','Interpreter','latex','FontSize',12);
        ylabel('Relative Error','Interpreter','latex','FontSize',12);
        ylim([0,max(1,max(errorRe.xie_shiftscaled(i,:)))]); grid on; 
        xlim([paramsarr(i,1),paramsarr(i,end)]); colororder(colorSet);
end
    leg = legend(txtleg2,'Orientation','Horizontal','FontSize',9);
    leg.Layout.Tile = 'north';
    txt = 'Kappa Bump-on-Tail Error in $\Omega(p)$ around';
    sgtitle({txt,txtbase},'Interpreter','latex','FontSize',16);
...
parfor j = 1:N^2
    ...
    xie = KappaBoT_Disp_Using_Xie(randparams(1)*randparams(2),1,randparams(3)/randparams(2),0,(randparams(5)-randparams(4))/randparams(2),randparams(6),kappa,xi_guess)*randparams(1)*randparams(2) + randparams(4)*randparams(1);
    dielectric = KappaBoT_dielectric(randparams(1),randparams(2),randparams(3),0,randparams(5)-randparams(4),randparams(6),kappa,init_guess) + randparams(4)*randparams(1);
...
errorSTD = std(abs([errorRand.omega]));
% toc
...
txt1 = ['Kappa BoT ($\kappa=$',num2str(kappa),'), ',num2str(var*100),'\% variation on'];
